...
class TestRoutesWithMockData(unittest.TestCase):
    @patch('src.web.app.ArbitrageEngine')
    def setUp(self, MockArbitrageEngine):
        # Patch ArbitrageEngine in app to prevent real constructor call
        mock_engine_instance = MockArbitrageEngine.return_value
    ...
    def test_get_positions_open(self, MockArbitrageEngine):
        ...
        print("Mock data fetched for open positions:", data)
    ...
    def test_get_positions_closed(self, MockArbitrageEngine):
        ...
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.data)
        print("Mock data fetched for closed positions:", data)
        self.assertEqual(len(data), 1)
        self.assertEqual(data[0]['symbol'], 'ETH/USDT')
        self.assertEqual(data[0]['status'], 'closed')

    def test_dashboard_with_mock_data(self):
        ...
        mock_chart.to_json.return_value = '{}'
        mock_engine.generate_spread_chart.return_value = mock_chart

        with self.app.app_context():
            self.app.engine = mock_engine
            response = self.client.get('/')
        print("Mock data fetched for dashboard route:", response.data.decode('utf-8'))
        self.assertEqual(response.status_code, 200)
